...
def _project_fc_network_weight_matrices(model, max_norm, eps=1e-8):
...
def estimate_wasserstein_kantorovich_rubinstein(samples_p, samples_q, device, eval_steps=1000,
    ...
    h = _project_fc_network_weight_matrices(h, 1)
    ...
    p_dataset = TensorDataset(samples_p)
    q_dataset = TensorDataset(samples_q)

    p_loader = DataLoader(p_dataset, batch_size=bs, shuffle=True, drop_last=True)
    q_loader = DataLoader(q_dataset, batch_size=bs, shuffle=True, drop_last=True)

    eval_p_loader = DataLoader(p_dataset, batch_size=eval_bs, shuffle=True, drop_last=False)
    eval_q_loader = DataLoader(q_dataset, batch_size=eval_bs, shuffle=True, drop_last=False)

    p_iter = iter(p_loader)
    q_iter = iter(q_loader)

    for i in trange(steps):
        p_samples, p_iter = _get_batch(p_iter, p_loader)
        q_samples, q_iter = _get_batch(q_iter, q_loader)
